...
def login_admin(app, config):
    app.login(username=config["web"]["user"], password=config["web"]["password"]) #take content from web by key
    yield
    app.logout()
...
filename = os.path.join((os.path.dirname(os.path.abspath(__file__))), "groups_data.json")#full path to file with filename
